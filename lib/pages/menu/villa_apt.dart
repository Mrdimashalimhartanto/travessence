...
class VillaApt extends StatefulWidget {
  const VillaApt({super.key});

  @override
  State<VillaApt> createState() => _VillaAptState();
}

class _VillaAptState extends State<VillaApt> {
  String fromCity = "Mau nginep dimana";
  String toCity = "Mau ke mana ?";
  DateTime departDate = DateTime(2025, 3, 11);
  bool isRoundTrip = false;
  int passengers = 1;
  String cabin = "Ekonomi";
  int selectedListKotaIndex = 0;

  // LIST PESAWAT
  List<VillaModel> get filterVilla {
    final selectedVilla = listVillaItems[selectedListKotaIndex];

    if (selectedVilla == "Bali") {
      return villaList;
    }

    return villaList
        .where((listVilla) => listVilla.category == selectedVilla)
        .toList();
  }
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  // _promoSection(),
                  // Menu di Section Villa & Hotel
                  menuSectionHotelVilla(context),
                  ...
                  SizedBox(height: 10),
                  categoriDestinasi(
                    items: listVillaItems,
                    selectedIndex: selectedListKotaIndex,
                    onTap: (index) {
                      setState(() {
                        selectedListKotaIndex = index;
                      });
                    },
                  ),
                  SizedBox(height: 8),
                  SingleChildScrollView(
                    ...
                    child: Row(
                      children: filterVilla
                          .map((villaapt) => VillaCard(villaapt: villaapt))
                          .toList(),
                    ),
                  ),
                  SizedBox(height: 19),
  ...
  Widget menuSectionHotelVilla(BuildContext context) {
    return Column(
      children: <Widget>[
        ...
        /// ROW 1
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween, // rata semua
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            GestureDetector(
              onTap: () => Navigator.pushNamed(context, '/akomodasi'),
              child: SizedBox(
                width: 90, // FIXED WIDTH supaya rata
                child: MenuVilla(
                  title: "Top Picks",
                  image:
  ...
  Widget penawaranCard({required String imageUrl}) {
    return Container(
